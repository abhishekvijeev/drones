...
{
	printf ("process_profile: profilename=%s\n", profilename);
	int retval = 0;
	...
	if (profilename && option != OPTION_REMOVE) {
		...
		if (!force_complain && pc) {
			...
			printf ("process_profile: cachename=%s\n", cachename);
			if (!cachename) {
	...
	printf ("process_profile: yyparse() starting\n");
	//processes the grammer and fills ProfileList
	retval = yyparse();
	printf ("process_profile: yyparse() finished with return value=%d\n", retval);
	if (retval != 0)
		goto out;
	...
	if (profile_ns)
		skip_cache = 1;
	// printf ("process_profile: profile_ns=%s\n", profile_ns);
	if (cachename) {
