...
void post_process_file_entries(Profile *prof)
{
	...
	// struct cod_entry *entr;
	// printf ("post_process_file_entries: begins\n");
	// list_for_each(prof->entries, entr)
	// {
	// 	printf ("name=%s, nt_name=%s\n", entr->name, entr->nt_name);
	// }
	// printf ("post_process_file_entries: ends\n");
...
int post_process_profile(Profile *profile, int debug_only)
{
	if (profile->current_domain != NULL)
		printf ("current domain not null, value is %s, len(domain_name) = %d, allow=%d\n", profile->current_domain->domain,strlen(profile->current_domain->domain),profile->current_domain->allow_cnt);
	else
		printf ("current domain NULL\n");
	
	if (profile->allow_net_domains != NULL)
	{
		printf ("Allowed net domains:\n");
	...
	/*
		creates a cod_entry with write access to /proc/{process id}/attr/current
		and adds that cod_entry to profile->entries	
	*/
	...
	/*
		...
		still don't know how exactly it works :P
	*/
...
int post_process_policy_list(ProfileList &list, int debug_only)
{
	int error = 0;
	for (ProfileList::iterator i = list.begin(); i != list.end(); i++) {
		printf ("**** Starting of post_process_profile: ****\n");
		error = post_process_profile(*i, debug_only);
		printf ("**** End of post_process_profile: ****\n");
		if (error)
			break;
	}
