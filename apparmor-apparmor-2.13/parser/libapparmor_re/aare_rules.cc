...
void *aare_rules::create_dfa(size_t *size, int *min_match_len, dfaflags_t flags)
{
	...
	// printf ("create_dfa:\n");
	// root->dump_syntax_tree(std::cout);
	// printf ("\n");
	*min_match_len = root->min_match_len();
	...
	try {
		...
		// printf ("create_dfa:\n");
		// printf ("dump_diff_encode:\n");
		// dfa.dump_diff_encode(std::cout);
		// printf ("dump:\n");
		// dfa.dump(std::cout);
		// printf ("chfa:\n");
		// chfa.dump(std::cout);
	...
	*size = buf->in_avail();
	...
	// printf ("aare_rules::create_dfa: dfa table:\n");
	// // fwrite (buffer, 1, strlen(buffer), stdout);
	// hexDump ("buffer", buffer, 100);
	return buffer;
}
