...
int expand_entry_variables(char **name)
{
	...
	struct var_string *split_var;
	int ret;
	// printf ("expand_entry_variables: name=%s\n", *name);
	assert(name);
	...
	while ((split_var = split_out_var(*name))) {
		...
		// printf ("prefix=%s, var=%s, suffix=%s\n", split_var->prefix, split_var->var, split_var->suffix);
...
int process_profile_variables(Profile *prof)
{
	 ...
	 */
	/*
	splits on '@' on prof->name and prof->attachment
	example:
		expand_entry_variables: name=@{PROC}/@{pid}/{maps,auxv,status}
		prefix=(null), var=PROC, suffix=/@{pid}/{maps,auxv,status}
		prefix=/proc//, var=pid, suffix=/{maps,auxv,status}
