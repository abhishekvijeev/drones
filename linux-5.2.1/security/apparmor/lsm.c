				   ...
				   struct sockaddr *address, int addrlen)
{
	...
	if(!unconfined(cl) && ctx != NULL && ctx->label != NULL)
	{
		...
		if (curr_domain != NULL)
		{
			...
			{   
				...
				{
					ret_val = 1;
					printk(KERN_INFO "apparmor_socket_connect: Packet from localhost to localhost allowed by process %s, pid %d\n", current->comm, current->pid);
				}
				...
				{
					ret_val = 1;
					printk(KERN_INFO "apparmor_socket_connect: DDS Multicast allowed %pi4, by process %s, pid %d\n", &daddr, current->comm, current->pid);
				}
				...
				{
					ret_val = 1;
					printk(KERN_INFO "apparmor_socket_connect: Multicast address allowed %pi4, by process %s, pid %d\n", &daddr, current->comm, current->pid);
				}
				...
				*/
				else
				{
					...
					if(allow)
						ret_val = 1;
					printk(KERN_INFO "apparmor_socket_connect: Domain declassification for message from process %s, pid %d, to address %pi4, flow is %d\n", current->comm, current->pid, &daddr, allow);
				}
				if (ret_val == 0)
					error = 0;
			...
			printk (KERN_INFO "apparmor_socket_connect sk->sk_family == AF_INET NOT SATISFIED: current process %s, pid %d, sk_family=%d, sock->type=%d\n", 
	...
	if (error == 0)
	{
		printk (KERN_INFO "apparmor_socket_connect: return is -13\n");
		return -EACCES;
	}
...
static int apparmor_socket_accept(struct socket *sock, struct socket *newsock)
{
		...
		if (curr_domain != NULL && label->pid != 0)
		{
			...
			if(sock->sk->sk_family == AF_INET)
			{
				...
				if (sender && sender_pid != current->pid)
				{
					...
					printk (KERN_INFO "apparmor_socket_accept: current process = %s, pid = %d, sent from process %s, pid = %d, Match is %d\n", 
								current->comm, current->pid, process_comm, label->pid, allow);
				} //end of if (curr_domain != NULL)
				else
				{
					printk (KERN_INFO "apparmor_socket_accept: Error in finding sender task struct, pid %d, current process %s\n", sender_pid, current->comm);
				}
			...
			printk (KERN_INFO "apparmor_socket_accept IF NOT SATISFIED: current process %s, pid %d, sk_family=%d, sock->type=%d\n", 
								current->comm, current->pid, sk_fam, socktype);
	...
	if (error == 0)
	{
		...
		printk (KERN_INFO "apparmor_socket_accept: return is -13, status of drop_flag = %d\n", drop_flag);
		return -EACCES;
	}
				   ...
				   struct msghdr *msg, int size, int flags)
{
	...
	// 	else
	// 		printk(KERN_INFO "apparmor_socket_recvmsg: Unable to obtain domain for process %s\n", current->comm);

	// 	// Extract the sending process' PID from the socket's label
	// 	sk_ctx = SK_CTX(sock->sk);
	// 	sk_label = aa_get_label(sk_ctx->label);
	// 	sender_pid = sk_label->pid;
	// 	aa_put_label(sk_ctx->label);
		
	// 	// Obtain the task_struct of the sending process from its PID
	// 	sender_process = pid_task(find_vpid(sender_pid), PIDTYPE_PID);

	// 	if(sender_process && recv_domain)
	// 	{
