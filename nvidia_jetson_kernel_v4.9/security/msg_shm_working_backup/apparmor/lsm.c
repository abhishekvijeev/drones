				   ...
				   struct msghdr *msg, int size)
{
	...
	if(curr_profile != NULL && !unconfined(curr_profile) && ctx != NULL && ctx->profile != NULL)
	{
		...
		if (curr_domain != NULL)
		{
			...
			{
				// struct unix_sock *u_sock = unix_sk(sk);
				// struct sock *unix_peer = u_sock->peer;
				// if(unix_peer)
				// {
				// 	struct aa_sk_ctx *peer_ctx = SK_CTX(unix_peer);
				// 	struct aa_profile *peer_profile = aa_get_newest_profile(peer_ctx->profile);
				...
				// else
				// {
				// 	printk(KERN_INFO "sendmsg: unix_socket - peer is NULL\n");
				// }
